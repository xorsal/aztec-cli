...
/**
 * Wallet context returned after initialization.
 */
...
export async function getOrDeployWallet(
  ...
  const { account, secretKey } = await createAccountFromPassphrase(
    testWallet,
    passphrase,
  );
  ...
  if (!deploy) {
    throw new Error(
      `Account ${accountAddress.toString()} is not deployed. Use --deploy to deploy it.`,
    );
  }
